...
class DaeExporter:
    ...
    def export_material(self, material, double_sided_hint=True):
        ...
        if emission_tex != None:
            ...
            self.writel(
                S_FX, 6, "<color>" + numarr(material.diffuse_color, material.emit) + " </color>"
    ...
    def export_mesh(self, node, armature=None, skeyindex=-1, skel_source=None, custom_name=None):
        ...
        if skeyindex == -1 and mesh.shape_keys != None and len(mesh.shape_keys.key_blocks):
            ...
            self.writel(
                S_MORPH,
                4,
                '<IDREF_array id="' + mid + '-morph-targets-array" count="' + str(len(morph_targets) - 1) + '">',
            )
            marr = ""
            warr = ""
            ...
            self.writel(
                S_MORPH,
                4,
                '<float_array id="' + mid + '-morph-weights-array" count="' + str(len(morph_targets) - 1) + '" >',
            )
        ...
        self.writel(
            S_GEOM,
            4,
            '<accessor source="#' + meshid + '-positions-array" count="' + str(len(vertices)) + '" stride="3">',
        )
        ...
        self.writel(S_GEOM, 4, "<technique_common>")
        self.writel(
            S_GEOM, 4, '<accessor source="#' + meshid + '-normals-array" count="' + str(len(vertices)) + '" stride="3">'
        )
        self.writel(S_GEOM, 5, '<param name="X" type="float"/>')
        self.writel(S_GEOM, 5, '<param name="Y" type="float"/>')
        self.writel(S_GEOM, 5, '<param name="Z" type="float"/>')
        ...
        if has_tangents:
            ...
            self.writel(
                S_GEOM,
                4,
                '<accessor source="#' + meshid + '-tangents-array" count="' + str(len(vertices)) + '" stride="3">',
            )
            self.writel(S_GEOM, 5, '<param name="X" type="float"/>')
            self.writel(S_GEOM, 5, '<param name="Y" type="float"/>')
            self.writel(S_GEOM, 5, '<param name="Z" type="float"/>')
            ...
            self.writel(
                S_GEOM,
                4,
                '<accessor source="#' + meshid + '-bitangents-array" count="' + str(len(vertices)) + '" stride="3">',
            )
            self.writel(S_GEOM, 5, '<param name="X" type="float"/>')
            self.writel(S_GEOM, 5, '<param name="Y" type="float"/>')
            self.writel(S_GEOM, 5, '<param name="Z" type="float"/>')
        ...
        if has_colors:
            ...
            self.writel(
                S_GEOM,
                4,
                '<accessor source="#' + meshid + '-colors-array" count="' + str(len(vertices)) + '" stride="3">',
            )
            self.writel(S_GEOM, 5, '<param name="X" type="float"/>')
            self.writel(S_GEOM, 5, '<param name="Y" type="float"/>')
            self.writel(S_GEOM, 5, '<param name="Z" type="float"/>')
        ...
        for m in surface_indices:
            ...
            if mat != None:
                ...
                self.writel(
                    S_GEOM, 3, "<" + prim_type + ' count="' + str(int(len(indices))) + '" material="' + matref + '">'
                )  # todo material
    ...
    def export_armature_bone(self, bone, il, si):
        ...
        self.writel(
            S_NODES, il, '<node id="' + boneid + '" sid="' + bonesid + '" name="' + bone.name + '" type="JOINT">'
        )
        il += 1
    ...
    def export_camera_node(self, node, il):
        ...
        if camera.type == "PERSP":
            ...
            self.writel(
                ...
                5,
                "<aspect_ratio> "
                + str(self.scene.render.resolution_x / self.scene.render.resolution_y)
                + " </aspect_ratio>",
            )
    ...
    def export_curve(self, curve):
        ...
        self.writel(
            S_GEOM,
            4,
            '<accessor source="#' + splineid + '-positions-array" count="' + str(len(points) / 3) + '" stride="3">',
        )
        self.writel(S_GEOM, 5, '<param name="X" type="float"/>')
        self.writel(S_GEOM, 5, '<param name="Y" type="float"/>')
        self.writel(S_GEOM, 5, '<param name="Z" type="float"/>')
        ...
        self.writel(
            S_GEOM,
            4,
            '<accessor source="#' + splineid + '-intangents-array" count="' + str(len(points) / 3) + '" stride="3">',
        )
        self.writel(S_GEOM, 5, '<param name="X" type="float"/>')
        self.writel(S_GEOM, 5, '<param name="Y" type="float"/>')
        self.writel(S_GEOM, 5, '<param name="Z" type="float"/>')
        ...
        self.writel(
            S_GEOM,
            4,
            '<accessor source="#' + splineid + '-outtangents-array" count="' + str(len(points) / 3) + '" stride="3">',
        )
        self.writel(S_GEOM, 5, '<param name="X" type="float"/>')
        self.writel(S_GEOM, 5, '<param name="Y" type="float"/>')
        self.writel(S_GEOM, 5, '<param name="Z" type="float"/>')
        ...
        self.writel(
            S_GEOM,
            4,
            '<accessor source="#' + splineid + '-interpolations-array" count="' + str(len(interps)) + '" stride="1">',
        )
        ...
        self.writel(S_GEOM, 4, "<technique_common>")
        self.writel(
            S_GEOM, 4, '<accessor source="#' + splineid + '-tilts-array" count="' + str(len(tilts)) + '" stride="1">'
        )
    ...
    def export_animation_transform_channel(self, target, keys, matrices=True):
        ...
        self.writel(
            S_ANIM, 4, '<accessor source="#' + anim_id + '-input-array" count="' + str(frame_total) + '" stride="1">'
        )
            ...
            for node in self.scene.objects:
                ...
                if allowed != None and not (node in allowed):
                    if (
                        node.type == "MESH"
                        and node.data != None
                        and (node in self.armature_for_morph)
                        and (self.armature_for_morph[node] in allowed)
                    ):
                ...
                if (
                    node.type == "MESH"
                    and node.data != None
                    and node.data.shape_keys != None
                    and (node.data in self.mesh_cache)
                    and len(node.data.shape_keys.key_blocks)
                ):
    ...
    def export_animations(self):
        ...
        for s in self.skeletons:  # workaround by ndee
            ...
            tmp_mat.append(
                [Matrix(s.matrix_local), tmp_bone_mat]
            )  # workaround by ndee -> stores skeleton and bone transformations
        ...
        if self.config["use_anim_action_all"] and len(self.skeletons):
            ...
            for x in bpy.data.actions[:]:
                ...
                for i, y in enumerate(self.skeletons):  # workaround by ndee
                    if y.animation_data:
                        ...
                        for j, bone in enumerate(s.pose.bones):  # workaround by ndee
                            bone.matrix_basis = (
                                Matrix()
                            )  # workaround by ndee -> resets the bone transformations. Important if bones in follwing actions miss keyframes
                ...
                self.writel(
                    S_ANIM_CLIPS,
                    1,
                    '<animation_clip name="' + x.name + '" start="' + str(start) + '" end="' + str(end) + '">',
                )
                for z in tcn:
            ...
            for i, s in enumerate(self.skeletons):  # workaround by ndee
                ...
                if s in cached_actions:
                    ...
                    for j, bone in enumerate(s.pose.bones):  # workaround by ndee
                        bone.matrix_basis = tmp_mat[i][1][
                            j
                        ]  # workaround by ndee  -> resets the bone transformation to what they were before exporting.
        else:
