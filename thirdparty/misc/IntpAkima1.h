...
class InterpolationAkima1
{
protected:
    // Construction (abstract base class).
    InterpolationAkima1(int quantity, Real const* F)
        :
        mQuantity(quantity),
        mF(F)
    {
        ...
        ERR_FAIL_COND_MSG(mQuantity < 3, "Invalid input to InterpolationAkima1 constructor.");
...
public:
    // Abstract base class.
    virtual ~InterpolationAkima1() = default;
    ...
    Real ComputeDerivative(Real* slope) const
    {
        if (slope[1] != slope[2])
        {
            if (slope[0] != slope[1])
            {
                if (slope[2] != slope[3])
                {
                    Real ad0 = Math::absf(slope[3] - slope[2]);
                    Real ad1 = Math::absf(slope[0] - slope[1]);
